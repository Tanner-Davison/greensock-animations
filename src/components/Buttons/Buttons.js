...
export const GlobalLinkButton = ({children, onClick, color,bgColor,align, onMouseOver,onMouseLeave, isHover}) =>{
  const handleClick = (e) => {
    onClick && onClick(e);
  };
return <GlobalButton $hover={isHover} onMouseLeave={onMouseLeave} onMouseOver={onMouseOver} $color={color} $align={align} $bgcolor={bgColor} onClick={handleClick}>
  {children}
  <LinkArrow id={'LinkArrow'} as={RightArrow} $color={color} $hover={isHover}/>
  </GlobalButton>
}
...
const LinkArrow = styled.svg`
  ...
  transform: ${(props) =>
      props.$hover ? `translateX(5px)` : "unset"};
